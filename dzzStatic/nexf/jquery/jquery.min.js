...
window.YLApp = {
     ...
     * @param data 鏁版嵁
     * @param target 鍙戦€佺洰鏍囷紝鍙互鏄崟涓猧d锛宨d鏁扮粍锛屾垨鑰卼rue锛堝彂閫佺粰鎵€鏈夌獥鍙ｏ級
     * */
    ...
    onReady: function(cb) {
        if (this._cbReady === false) return; //鍙厑璁竢eady涓€娆�
        if (!cb) {
            cb = function() {}
        }
        this._cbReady = cb;
    ...
    hashBugForIeFix: function() {
        document.body.focus(); //hash鐨刡ug锛屽吋瀹笽E
    },
    ...
    switch (msg.type) {
        case "ylui-ping":
            if (YLApp.id) {
                ...
                if (YLApp._cbReady) {
                    var relCbReady = YLApp._cbReady(); //鎵цready鐨勫洖璋�
                    YLApp._cbReady = false; //娓呯┖ready
                    if (relCbReady !== false)
                        YLApp.emit('ready', null, true); //鍙戦€乺eady浜嬩欢
                }
                if (!YLApp._created) {
...
var _nex_window_open = window.open;
window.open = function(u, n, w) {
    if (YLApp.id != '') {
        if (u.indexOf('//') == -1) {
            return YLApp.open('http://119.45.134.67:8888/dzzoffice/' + u)
        } else {
            return YLApp.open(u)
        } //是完整url
    } else {
        return _nex_window_open(u, n, w)
    }
}